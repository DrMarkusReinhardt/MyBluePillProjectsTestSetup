...
// The Slot base class, its template parameter indicates the data type of the parameters it expects to receive.
// Slots can only be connected to Signals with identical ParameterTypes.
template<class ParameterType>
  ...
  public:
    ...
    // Allows the slot to be called by the signal during emission
    virtual void
    operator() (ParameterType param) = 0;

    // Allows the signal to take a copy of the slot so that it can maintain an internal reference to it upon connection.
    // Essentially a virtual copy constructor.
    virtual Slot<ParameterType>*
    Clone () = 0;
  };
...
// an interrupt so the recipient will want to be fairly quick about how they process it.
template<class ParameterType, int slots = 8>
  ...
  public:
    ...
    void
    Connect (Slot<ParameterType>& slot)
    {
...
template<class ParameterType>
  class FunctionSlot : public Slot<ParameterType>
  {
    typedef void
    (*FunctPtr) (ParameterType);

    // A free standing function pointer
    FunctPtr funct;

  public:
    FunctionSlot (FunctPtr _funct) :
	funct (_funct)
    {
    }
...
template<class ObjectType, class ParameterType>
  class MethodSlot : public Slot<ParameterType>
  {
    typedef void
    (ObjectType::*FunctPtr) (ParameterType);
  ...
  public:
    MethodSlot (ObjectType *_obj, FunctPtr _funct) :
	obj (_obj), funct (_funct)
    {
    }
